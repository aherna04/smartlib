...
mytempyahoo3=/var/tmp_ram/yweather3.out
...
/g'  -e 's/^ *//g' -e 's/\"//g' -e 's/\"//g'| awk 'BEGIN{FS="=";OFS=":";}{print $1,$2}' | grep  -v "day" | grep -v ^\: | grep -v "date" > $mytempyahoo3
	...
	codeTomorrow=`cat $mytempyahoo3 | grep code | head -2 | tail -1 | cut -d":" -f2`
        codeAfterTomorrow=`cat $mytempyahoo3 | grep code | head -3 | tail -1 | cut -d":" -f2`
	...
	conditionTomorrow=`cat $mytempyahoo3		|grep -e text| head -2 | tail -1 | cut -d":" -f2`
	conditionAfterTomorrow=`cat $mytempyahoo3	|grep -e text| head -3 | tail -1 | cut -d":" -f2`
